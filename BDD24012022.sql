...
-- -----------------------------------------------------
-- Table `menuiz`.`T_D_HISTORIQUE_SAV_HSV`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `menuiz`.`T_D_HISTORIQUE_SAV_HSV` ;

CREATE TABLE IF NOT EXISTS `menuiz`.`T_D_HISTORIQUE_SAV_HSV` (
  `HSV_ID` INT NOT NULL AUTO_INCREMENT,
  `HSV_DSV_ID` INT NOT NULL,
  `HSV_ETAT_CHANGE` INT NOT NULL,
  `HSV_DATE_CHANGE` DATE NOT NULL,
  PRIMARY KEY (`HSV_ID`),
  CONSTRAINT `fk_T_D_HISTORIQUE_SAV_HSV_T_D_Dossier_SAV_DSV1`
    FOREIGN KEY (`HSV_DSV_ID`)
    REFERENCES `menuiz`.`T_D_Dossier_SAV_DSV` (`DSV_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
